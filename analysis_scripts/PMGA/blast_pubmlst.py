...
"NEIS0351","NEIS1364","NEIS1969","NEIS2109","NadA_peptide","fuck"]
...
serotypes = {
	"a":{"essential":["acs3","acs4","bexA","bexB","bexC","bexD","hcsA","hcsB"],"nonessential":["bcs1","bcs2"]},
    	"b":{"essential":["bcs1","bcs2","bcs3","bcs4","bexA","bexB","bexC","bexD","hcsA","hcsB"],"nonessential":[]},
    	"c":{"essential":["fcs1","ccs2","ccs3","ccs4","bexA","bexB","bexC","bexD","hcsA","hcsB"],"nonessential":[]},
    	"d":{"essential":["ecs1","ecs2","dcs3","dcs4","ecs8","bexA","bexB","bexC","bexD","hcsA","hcsB"],"nonessential":[]},
...
ST_unique = {
		"acs1":"a","acs2":"a","acs3":"a","acs4":"a",
	    	"bcs3":"b","bcs4":"b",
            	...
            	"ecs3":"e","ecs4":"e","ecs5":"e","ecs6":"e","ecs7":"e",
	    	"fcs2":"f","fcs3":"f",}
...
def generate_sg_predictions(data):
	...
	for query in sorted(data):
		...
		elif "Haemophilus influenzae" in species:
			species == "hinfluenzae"
...
def count_loci_per_scheme(scheme_data,results_dict):
	...
	with open(os.path.join(out_path,"loci_counts.tab"),"w") as f:
		...
		for i in range(0,max_length):
			f.write("{}_{}\t".format(scheme_counter[i].replace(" ","_").replace(".","_").replace(":","_").replace("__","_"),len(scheme_data[scheme_counter[i]])))
		f.write("\n")
		for in_file in loci_counts:
			if in_file not in locus_counts:
				locus_counts[in_file] = {}
			f.write("{}\t".format(in_file))
			for i in range(0,max_length):
				if scheme_counter[i] in loci_counts[in_file]:
					f.write("{}\t".format(len(loci_counts[in_file][scheme_counter[i]])))
					if scheme_counter[i] not in locus_counts[in_file]:
						locus_counts[in_file][scheme_counter[i].replace(" ","_").replace(".","_").replace(":","_").replace("__","_")] = len(loci_counts[in_file][scheme_counter[i]])
				else:
					f.write("0\t")
			f.write("\n")
	with open(os.path.join(out_path,"scheme_counts.json"),"w") as f:
		json.dump(locus_counts,f)			
		...
		for contig in results_dict[in_file]["contigs"]:
			for allele in results_dict[in_file]["contigs"][contig]:
				if results_dict[in_file]["contigs"][contig][allele]:		
					for hit in results_dict[in_file]["contigs"][contig][allele]:
						region_type = hit["region_type"]
						if region_type == "CDS" or region_type == "IGR":
							allele_name = hit["allele_name"]
							cov = hit["cov"]
							allele_id = hit["allele_id"]
							seq = hit["qseq"]
...
def main():	
	...
	group.add_argument('-x','--xlsx',help="Excel file with Filename column and optional Species column ({})".format(', '.join([k for k in species_dict.keys()]))) 
	...
	parser.add_argument('-scf','--schemefile',help="Provide existing scheme json file to load")
	...
	if isinstance(args["indir"],str):
		...
		file_frame = pd.read_excel(args['xlsx'], engine='openpyxl')
	...
	if args["schemefile"]:
		scheme_file = args["schemefile"]
	else:
		scheme_file = None
	scheme_data = get_schemes(scheme_file)	
	print("#######################################################################################################")	
