...
# version 1.5.7 - Added notes about composite coverage
# 
# CW, 2/20/25 - Updated for RHEL8 migration
# CW, 6/6/25 - Updated for Git
#######

script_version = 2.0
script_subversion = 2 ##Updated script for new pandas
...
from BLASThelpers_RHEL8 import BLASTheaders as bh
import BLASThelpers_RHEL8 
...
_outfmt_str, _outfmt_head = BLASThelpers_RHEL8.BLASTtableCommandAndHeaders(_BLASTheaderList)
...
class PrimaryExporter:
    ...
    def __init__(self,parent,export_template_file,export_reference_file,lookupDir):
        ...
        for (value, header) in self.headers.items():
            if header not in self.export_template.columns:
                print("Warning: cannot find header for {}. Should be {}".format(value,header)) 
        #MLST
    ...
    def findST(self,Identifier,allele_series):
        ...
        thisST = {'clonal_complex':None} ##Failure to set this results in a spurious match between thisSt and table STs with no Clonal Complex. 
        ...
        try:
            ...
            if result_set == null_set:
                ...
                ST_ID = 'New'
            thisST_series = pd.Series(thisST) #ensure 'thisST' is a pd series
            profile_filtered_aligned, thisST_aligned = profile_filtered.align(thisST_series, axis=1) #align df on columns
            profile_filtered['matches'] = np.sum(profile_filtered_aligned == thisST_aligned, axis=1) # pylint: disable=no-member
...
class LocusExtractor:
    ...
    def bestAlleleByBlast(self,gene,query_file,genome_info,is_DNA=True, is_subregion=False):
        ...
        ##Low identity is never valid (don't want to hit homolog).         
        ...
        if len(perfect_hits) > 0:
            ...
            if len(results) == 0: 
                ...
                if sum(high_ident_hits) > 0:
                    ...
                    near_edge = (results['min_distance_to_edge'] < _warning_distance_edge) 
    ...
    ###Large routine for handling loci with specific peptide subregions of interest. 
    ...
    def findAntigenSubregion(self,gene,subregions,genomeInfo,primer_regions):
        ...
        if query_file is not None:
            ...
            if best is not None:
                if expectedORF:
                    locusBLAST,orf_seq = BLASThelpers_RHEL8.convertHitToORF(best,allele_seq,gene)
                else:
            ...
            for pep in subregions:
                ...
                if bestPEP is None:
                    ...
                    if best is None:
                        ...
                        if not BLASThelpers_RHEL8.hitContainsAnother(locusBLAST,bestPEP):  
                            if BLASThelpers_RHEL8.hitContainsAnother(best, bestPEP): ##not in trimed locus
...
if __name__ == "__main__":
    ...
    has_BLAST = BLASThelpers_RHEL8.test_executables()
