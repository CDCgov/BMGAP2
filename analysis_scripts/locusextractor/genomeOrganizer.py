...
def assemblySetup(assembly_file,relative_path=None,ValidOnly=False): 
    try:
...
def moveListedGenomes(genome_frame,destination_dir,inventory_file,field_dict=None,extract_info=True):
    ...
    for f,g in genome_frame_trimmed.groupby('Filename'):
        g = g.copy()
        sample_name = g['Lab_ID'].unique().tolist()[0] 
    ...
    duplicates = inventory[prospective & dups].copy() 
    if len(duplicates) > 0:
...
def extractAssemblyStats(inventoryFrame,inventoryDir=None):
    ...
    return inventoryFrame, master_contig_inventory 
...
def submit(submit_args):
    ...
    if not inventory.valid:
        return 1
    inventory.exportAssemblies(isolates,export_dir,ActiveOnly=True,include_qual=True)
