...
##True if two hits point to same part of SUBJECT (as if searching with homologous queries)
...
#     assert external is not None, "Don't check for range of BLAST it if there is no hit!"
    ...
    ##Compare two Pandas.Series objects originating from BLAST results
# Returns TRue if first in inside of second 
def hitsOverlapQuery(hit1,hit2):
#     assert external is not None, "Don't check for range of BLAST it if there is no hit!"
    if (hit1 is None) or (hit2 is None):
        return False
    start1 = hit1[bh['qstart']]
    stop1 = hit1[bh['qend']]
    if start1 < stop1:
        low1 = start1
        high1 = stop1
    ...
    if start2 < stop2:
        ...
        low2 = stop2
        high2 = start2    
    return ((low1 < high2) and (high1 >= low2)) or ((low2 < high1) and (high2 >= low1))
