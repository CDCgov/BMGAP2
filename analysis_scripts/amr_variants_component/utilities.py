...
SCRIPT_VERSION = 1.11 #add 'fq' extension
    ...
    def write(self, message):
        ...
        if self.filename is not None:
            try:
                ...
                ##Print Would get into infinite exception loop
...
def addCurrentTimeToFilename(filename):
    return appendToFilename(filename,"_"+time.strftime("%Y%m%d%H%M%S"))
...
def checkForOverwrite(filename,force=False,_verbose=True):
    if os.path.isfile(filename):
        if force:
            ...
            if not renameWithTimestamp(filename): ##Unable to rename old file and move it out of the way
